...
#pragma warning restore 0649
		...
		private const string bucketName = "painting-augustus-bucket";
		private const string region = "us-east-2";
		private const string accessKey = "AKIAV4ELJ6VOS2HS6GA2";
		private const string secretKey = "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";
		private void Awake()
		{
        ...
        private void SaveRenderTextureToPNG(string textureName, RenderTexture renderTexture, Action<TextureImporter> importAction = null)
        {
            ...
            string path = Path.Combine(Application.dataPath, textureName + "_" + dateTime + ".png");
            if (path.Length != 0)
            {
                ...
                // Combine texture name with date and time
                string uniqueString = $"{textureName}_{dateTime}";
        ...
        public void UploadPngFile(string filePath, string textureName)
        {
            ...
            try
            {
                ...
                // Generate a unique key name using the current date and time
                string dateTime = DateTime.Now.ToString("yyyyMMdd_HHmmssfff");
                string keyName = $"uploads/{Path.GetFileNameWithoutExtension(filePath)}_{dateTime}.png";

                // Set up AWS credentials
                var credentials = new Amazon.Runtime.BasicAWSCredentials(accessKey, secretKey);
                var s3Client = new AmazonS3Client(credentials, RegionEndpoint.GetBySystemName(region));

                // Set up the PutObjectRequest for uploading
                var putRequest = new PutObjectRequest
                {
                    InputStream = new MemoryStream(pngBytes),  // The PNG file in a byte stream
                    Key = keyName,                            // The unique key for S3 object
                    BucketName = bucketName,                  // The target S3 bucket
                    ContentType = "image/png"                 // Content type for the file
                };

                // Use a callback with PutObjectAsync
                s3Client.PutObjectAsync(putRequest, (responseObj) =>
                {
                    if (responseObj.Exception != null)
                    {
                        Debug.LogError($"Failed to upload PNG: {responseObj.Exception.Message}");
                    }
                    else
                    {
                        Debug.Log($"Successfully uploaded {keyName} to {bucketName}");
                    }
                });
            }
            catch (Exception ex)
